...
class ParallelBackend(metaclass=Singleton):
    ...
    BACKEND_DICT = {"ray": RayBackend, "sequential": SequentialBackend,}

    def __init__(self):
        self.backend = None
        self.default_timeout = None

    def init(self, backend: str = "ray", n_workers: Optional[int] = None, n_cpus: Optional[int] = None, gpu_devices: Optional[List[int]] = None, default_timeout: float = -1, max_tasks_per_child: Optional[int] = None, worker_initializers: Optional[Union[List[Callable], Callable]] = None, ):
        ...
        self.backend = self.BACKEND_DICT[backend](n_workers=n_workers, n_cpus=n_cpus, gpu_devices=gpu_devices, max_tasks_per_child=max_tasks_per_child, worker_initializers=worker_initializers,)
        self.backend.init()
