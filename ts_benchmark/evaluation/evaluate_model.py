...
class EvalResult:
    ...
    def collect(self) -> Generator[pd.DataFrame, None, None]:
        ...
        min_interval = 0 if len(self.result_list) < 100 else 0.1  # improves visual effect
        for i, result in enumerate(
            ...
            if collector.get_size() > 100000:
                result_df = build_result_df(collector.collect(), self.model_factory, self.strategy)
                yield result_df
                collector.reset()
...
def eval_model(model_factory: ModelFactory, series_list: list, evaluation_config: dict) -> EvalResult:
    """
    ...
    if metric == "all":
        ...
        metric = [metric]
    metric = [{"name": metric_info} if isinstance(metric_info, str) else metric_info for metric_info in metric]
    # Check if the evaluation indicators are legal
    invalid_metrics = [m.get("name") for m in metric if m.get("name") not in strategy_class.accepted_metrics()]
    if invalid_metrics:
        raise RuntimeError("The evaluation index to be evaluated does not exist: {}".format(invalid_metrics))
    ...
    for series_name in tqdm.tqdm(series_list, desc=f"scheduling {model_factory.model_name}"):
        ...
        result_list.append(eval_backend.schedule(strategy.execute, (series_name, model_factory)))
    return EvalResult(strategy, result_list, model_factory, series_list)


def build_result_df(result_list: List, model_factory: ModelFactory, strategy: Strategy) -> pd.DataFrame:
    ...
    if missing_fields:
        raise ValueError("These required fields are missing in the result df: {}".format(missing_fields))
    return result_df
