...
class RollingForecastEvalBatchMaker:
    ...
    def __init__(self, series: pd.DataFrame, index_list: List[int]):
        self.series = series
    ...
    def make_batch_predict(self, batch_size: int, win_size: int) -> dict:
        ...
        """
        index_list = self.index_list[self.current_sample_count : self.current_sample_count + batch_size]
...
class RollingForecast(ForecastingStrategy):
    ...
    @staticmethod
    def _get_index(train_length: int, test_length: int, horizon: int, stride: int) -> List[int]:
        """
    ...
    def _get_split_lens(
        ...
        if train_length <= 0 or test_length <= 0:
            raise ValueError("The length of training or testing data is less than or equal to 0")
        return train_length, test_length

    def _execute(
        self, series: pd.DataFrame, meta_info: Optional[pd.Series], model_factory: ModelFactory, series_name: str
    ) -> List:
        """
    ...
    def _eval_sample(
        ...
        average_inference_time = float(total_inference_time) / min(len(index_list), num_rollings)
    ...
    def _eval_batch(
        self, series: pd.DataFrame, meta_info: Optional[pd.Series], model: ModelBase, series_name: str
    ) -> List:
        """
        ...
        all_test_results, target_norm_list, predicts_norm_list = [], [], []
        for predicts, target in zip(all_predicts, targets):
            single_series_results = self.evaluator.evaluate(target, predicts, eval_scaler, train_valid_data.values)
            target_norm_list.append(eval_scaler.transform(target))
            predicts_norm_list.append(eval_scaler.transform(predicts))
        ...
        # np.savez("Save_True_Pre.npz", targets=np.stack(target_norm_list), predicts=np.stack(predicts_norm_list))
        return single_series_results
