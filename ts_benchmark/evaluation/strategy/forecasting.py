...
class ForecastingStrategy(Strategy, metaclass=abc.ABCMeta):
    ...
    def execute(self, series_name: str, model_factory: ModelFactory) -> Any:
        ...
        data = data_pool.get_series(series_name)  ## 拿到数据了
        meta_info = data_pool.get_series_meta_info(series_name)
        # data.to_pickle(f'{series_name}.pkl')
        try:
            single_series_results = self._execute(data, meta_info, model_factory, series_name)
    ...
    def _get_eval_scaler(self, train_valid_data: pd.DataFrame, train_ratio_in_tv: float) -> Any:
        """
