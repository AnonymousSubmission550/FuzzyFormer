...
def load_record_data(record_files: List[str], drop_columns: Optional[List[str]] = None) -> pd.DataFrame:
    """
    ...
    record_files = itertools.chain.from_iterable(
        [[fn] if not os.path.isdir(fn) else find_record_files(fn) for fn in record_files]
    )
...
def save_log(result_df: pd.DataFrame, save_path, file_prefix: str, compress_method: str = "gz") -> str:
    """
    Save log data.
    ...
    if result_df["log_info"].any():
        ...
        for error in itertools.islice(error_itr, 3):
            logger.info(error)
        if any(error_itr):
            logger.info("-------------More error messages can be found in the record files!-------------")

    if save_path is not None:
        result_path = os.path.join(ROOT_PATH, "result", save_path) if not os.path.isabs(save_path) else save_path
    else:
        result_path = os.path.join(ROOT_PATH, "result")
    os.makedirs(result_path, exist_ok=True)
    file_path = os.path.join(result_path, file_prefix)
    print("**********************************************")
    print(f"Mae is {result_df['mae_norm'].values[0]:.3f}")
    print(f"Mse is {result_df['mse_norm'].values[0]:.3f}")
    print("**********************************************")
    return result_df.to_csv(file_path, index=False, encoding="utf-8")
