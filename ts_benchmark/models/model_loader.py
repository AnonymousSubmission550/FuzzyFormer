...
def import_model_info(model_path: str) -> Union[Dict, Callable]:
    ...
    if not isinstance(model_info, (Dict, Callable)):
        raise ValueError(f"Unsupported model info with type {type(model_info).__name__}")
...
def get_model_hyper_params(recommend_model_hyper_params: Dict, required_hyper_params: Dict, model_config: Dict) -> Dict:
    """
    ...
    missing_hp = set(required_hyper_params) - set(model_hyper_params)  # 是否把所有的参数都给定值了
    if missing_hp:
