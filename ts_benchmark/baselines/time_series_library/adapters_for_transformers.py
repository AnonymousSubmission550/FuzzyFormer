...
from ts_benchmark.baselines.time_series_library.mycode import FigPlot
...
class TransformerAdapter(ModelBase):
    ...
    def validate(self, valid_data_loader, criterion):
        ...
        for input, target, input_mark, target_mark in valid_data_loader:
            ...
            dec_input = torch.cat([target[:, : config.label_len, :], dec_input], dim=1).float().to(device)
    ...
    def forecast_fit(self, train_valid_data: pd.DataFrame, train_ratio_in_tv: float) -> "ModelBase":
        ...
        # for name, param in self.model.named_parameters():
        #     print(f"Parameter Name: {name} | Values: {param.data}")
        print(
            "----------------------------------------------------------",
            self.model_name,
        )
        ...
        for epoch in range(config.num_epochs):
            ...
            for i, (input, target, input_mark, target_mark) in enumerate(train_data_loader):
                ...
                dec_input = torch.zeros_like(target[:, -config.horizon :, :]).float()
                dec_input = torch.cat([target[:, : config.label_len, :], dec_input], dim=1).float().to(device)
                output = self.model(input, input_mark, dec_input, target_mark)  # 32, 24, 7
                target = target[:, -config.horizon :, :]
                output = output[:, -config.horizon :, :]
                # FigPlot(target.detach().cpu().numpy(), output.detach().cpu().numpy())
    ...
    def _perform_rolling_predictions(
        self, horizon: int, input_np: np.ndarray, all_mark: np.ndarray, device: torch.device
    ) -> list:
        """
