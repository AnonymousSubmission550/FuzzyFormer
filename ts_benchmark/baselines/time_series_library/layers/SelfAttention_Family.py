...
class FullAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None):
        B, L, H, E = queries.shape  # 32, 76, 8, 32
        sigma = (1 / E) ** (1 / E)
        _, S, _, D = values.shape
        scale = self.scale or 1.0 / sqrt(E)
        ...
        if self.mask_flag:
            if attn_mask is None:
                attn_mask = TriangularCausalMask(B, L, device=queries.device)
            scores.masked_fill_(attn_mask.mask, -np.inf)
        A = self.dropout(torch.softmax(scale * scores, dim=-1))  # torch.Size([32, 8, 76, 76])
        V = torch.einsum("bhls,bshd->blhd", A, values)  # torch.Size([32, 76, 8, 32])
        if self.output_attention:
            return V.contiguous(), A
        else:
            return V.contiguous(), None
...
#     def forward(self, queries, keys, values, attn_mask, tau=None, delta=None):
#         B, L, H, E = queries.shape  # 32, 104, 8, 32
#         _, S, _, D = values.shape
#         # fuzzy
#         # a = -1 / 2 * 1 / E * dist.pow(2).sum(dim=-1)
#         # cc = torch.softmax(a, dim=-1)
#         sigma = (1 / E) ** (1 / E)
#         queries = queries.permute(0, 2, 3, 1)
#         keys = keys.permute(0, 2, 3, 1)
#         values = values.permute(0, 2, 3, 1)
#         input_data_use = queries.unsqueeze(3).repeat(1, 1, 1, E, 1)
#         attention_center = keys.unsqueeze(2).repeat(1, 1, E, 1, 1)
#         values_con = values.unsqueeze(2).repeat(1, 1, E, 1, 1)
#         dist = (input_data_use - attention_center) / sigma
#         fire_strength = torch.exp(-1 / 2 * 1 / E * dist.pow(2).sum(dim=-1))
#         norm_fire_strength = fire_strength / (fire_strength.sum(dim=-1, keepdim=True))
#         V = torch.einsum("abcd, abcde->abce", norm_fire_strength, values_con).permute(0, 2, 3, 1)
#         if self.output_attention:
#             return V.contiguous(), None
#         else:
#             return V.contiguous(), None
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None):
        ...
        _, S, _ = keys.shape
        H = self.n_heads
        queries = self.query_projection(queries).view(B, L, H, -1)  # 32, 104, 8, 32
