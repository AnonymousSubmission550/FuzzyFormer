...
from ts_benchmark.baselines.fuzzy.utils.tools import EarlyStopping, adjust_learning_rate
from ts_benchmark.utils.data_processing import split_before
from typing import Type, Dict, Optional, Tuple
from torch import optim
from ts_benchmark.baselines.utils import forecasting_data_provider, train_val_split, get_time_mark
from ts_benchmark.baselines.fuzzy.models.fuzzy_model import FuzzyModel
...
class FUZZY(ModelBase):
    ...
    def required_hyper_params() -> dict:
        ...
        """
        return {"seq_len": "input_chunk_length", "horizon": "output_chunk_length", "norm": "norm"}
    ...
    def padding_data_for_forecast(self, test):
        ...
        date = pd.date_range(start=start, periods=self.config.horizon + 1, freq=self.config.freq.upper())
    ...
    def _padding_time_stamp_mark(self, time_stamps_list: np.ndarray, padding_len: int) -> np.ndarray:
        """
    ...
    def forecast_fit(self, train_valid_data: pd.DataFrame, train_ratio_in_tv: float) -> "ModelBase":
        ...
        self.model = FuzzyModel(self.config)
        print(
            "----------------------------------------------------------",
            self.model_name,
        )
        ...
        for epoch in range(config.num_epochs):
            ...
            for i, (input, target, input_mark, target_mark) in enumerate(train_data_loader):
                ...
                if math.isnan(loss_sum):
                    print("?")
            print(f"{epoch} Epoch Train Loss is {loss_sum/i:.3f}")
            if train_ratio_in_tv != 1:
                valid_loss = self.validate(valid_data_loader, criterion)
                print(f"{epoch} Epoch Valid Loss is {valid_loss:.3f}")
